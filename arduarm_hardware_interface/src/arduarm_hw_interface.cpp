...
void ArduarmHWInterface::callback(const std_msgs::Int32MultiArray::ConstPtr& msg){
    for(int i = 0; i < num_joints_ ; ++i){
      joint_position_[i] = msg->data[i] * M_PI / 180.0;
  }
}
  ...
  :ros_control_boilerplate::GenericHWInterface(nh, urdf_model)
{
  ...
  cmd_pub = nh_.advertise<std_msgs::Int32MultiArray>("/arduarm/pos_cmd", 1000);
